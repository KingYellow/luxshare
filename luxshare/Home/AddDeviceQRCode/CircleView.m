...
- (void)circleWithProgress:(NSInteger)progress andIsAnimate:(BOOL)animate{
    if (animate) {
        ...
        pathAnima.duration = progress/10.0f;
...
- (UILabel *)numbLb {
    if(_numbLb == nil) {
        ...
        int fontNum = self.bounds.size.width/4;
...
- (void)ddcircleWithProgress:(NSInteger)progress andIsAnimate:(BOOL)animate{
    if (animate) {
       
        _progressFlag = 0;
        _progressValue = progress;
        
        UIBezierPath *path = [UIBezierPath bezierPathWithArcCenter:CGPointMake(self.bounds.size.width/2, self.bounds.size.height/2) radius:self.bounds.size.width/2 - _strokelineWidth startAngle:-M_PI_2 + M_PI_2 * 99/50 endAngle:-M_PI_2 + M_PI*2 clockwise:YES];
