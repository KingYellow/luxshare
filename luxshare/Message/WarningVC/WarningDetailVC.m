...
@interface WarningDetailVC ()<UITableViewDelegate,UITableViewDataSource>
...
- (void)initConfig{
    ...
    self.navigationItem.title = QZHLoaclString(@"message_warningDetail");
    self.rightBtn =  [self exp_addRightItemTitle:QZHLoaclString(@"finish") itemIcon:@""];
    self.rightBtn.hidden = YES;
    [self UIConfig];
}
- (void)UIConfig{
    self.topView = [MessageTopView initmessagetopViewName:QZHLoaclString(@"warning")];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [cell.checkBtn addTarget:self action:@selector(gotoOnlineLive:) forControlEvents:UIControlEventTouchUpInside];
...
//    TuyaSmartMessageDetailListRequestModel *model = [[TuyaSmartMessageDetailListRequestModel alloc] init];
//    model.msgType = TYMessageTypeAlarm;
//    model.limit = 10;
//    model.offset = self.modelArr.count;
//    model.msgSrcId = self.msgSrcId;
    
//
//    [[TuyaSmartMessage new] fetchMessageDetailListWithListRequestModel:model success:^(NSArray<TuyaSmartMessageListModel *> * _Nonnull messageList) {
//
//
//    } failure:^(NSError *error) {
//
