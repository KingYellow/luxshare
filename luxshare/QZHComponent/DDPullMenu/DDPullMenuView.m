...
@interface DDPullMenuView()<QZFilterTableViewDelegate>
...
@property (nonatomic, strong) DDPullTabView *filterTableView;
...
+ (instancetype)conditionFilterViewWithListCount:(NSInteger)listCount FilterBlock:(FilterBlock)filterBlock {
    DDPullMenuView *conditionFilter = [[DDPullMenuView alloc] initWithFrame:CGRectMake(0, 0, QZ_SCREEN_WIDTH, 40)];
...
- (void)showTableView:(UIButton *)btn {
    ...
    if (_bgView == nil) {
        _bgView = [[UIView alloc] initWithFrame:CGRectMake(0, 0 , self.width, QZ_SCREEN_HEIGHT)];
    ...
    for (NSInteger i = 0; i<self.filterButtons.count; i++) {
        ...
        if (btn == filterButton) {
            ...
            self.filterTableView.frame = CGRectMake(0, self.bottom + QZHNAVI_HEIGHT, self.width, 0);
...
- (DDPullTabView *)filterTableView {
    if (!_filterTableView) {
        _filterTableView = [[DDPullTabView alloc] initWithFrame:CGRectZero];
    }
