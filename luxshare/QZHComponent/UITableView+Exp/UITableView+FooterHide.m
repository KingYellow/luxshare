...
#import "UITableView+FooterHide.h"
#import "MJRefresh.h"
#import <objc/runtime.h>


@implementation UITableView (FooterHide)
...
- (void)setFooterShow {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (!self.mj_footer.hidden) { // 没有隐藏
            actualHeight = actualHeight + self.mj_footer.frame.size.height + 10;
            // 默认的mj_footer高度为44  默认实际偏移了54
        }
        if (actualHeight >= heightOfContentSize) {
            // 实际显示高度大于内容高度 代表第一页都没有占满
