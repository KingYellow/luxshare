...
#define MBMainThreadAssert() NSAssert([NSThread isMainThread], @"LUXProgressHUD needs to be accessed on the main thread.");

CGFloat const LUXProgressMaxOffset = 1000000.f;

static const CGFloat LUXDefaultPadding = 4.f;
static const CGFloat LUXDefaultLabelFontSize = 16.f;
static const CGFloat LUXDefaultDetailsLabelFontSize = 12.f;


@interface LUXProgressHUD () {
    // Deprecated
...
@interface LUXProgressHUDRoundedButton : UIButton
@end


@implementation LUXProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    LUXProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    LUXProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (LUXProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            LUXProgressHUD *hud = (LUXProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = LUXProgressHUDAnimationFade;
    _mode = LUXProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(LUXProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == LUXProgressHUDAnimationZoom) {
        type = animatingIn ? LUXProgressHUDAnimationZoomIn : LUXProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == LUXProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == LUXProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == LUXProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == LUXProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    LUXProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<LUXProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    LUXBackgroundView *backgroundView = [[LUXBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = LUXProgressHUDBackgroundStyleSolidColor;
    ...
    LUXBackgroundView *bezelView = [LUXBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:LUXDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:LUXDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [LUXProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:LUXDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    else if (mode == LUXProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[LUXBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == LUXProgressHUDModeDeterminate || mode == LUXProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
    ...
    else if (mode == LUXProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[LUXProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[LUXProgressHUD class]]];
#endif
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [LUXBarProgressView appearanceWhenContainedIn:[LUXProgressHUD class], nil];
#else
        appearance = [LUXBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[LUXProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((LUXBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((LUXBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    LUXBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? LUXDefaultPadding : 0.f;
...
- (void)setMode:(LUXProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == LUXProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation LUXBarProgressView
...
@implementation LUXBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = LUXProgressHUDBackgroundStyleBlur;
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
            ...
            _style = LUXProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(LUXProgressHUDBackgroundStyle)style {
    if (style == LUXProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = LUXProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    LUXProgressHUDBackgroundStyle style = self.style;
    if (style == LUXProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == LUXProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation LUXProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [LUXProgressHUD allHUDsForView:view];
    for (LUXProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable LUXProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    LUXBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == LUXProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    MBMainThreadAssert();
    self.backgroundView.style = LUXProgressHUDBackgroundStyleSolidColor;
...
@implementation LUXProgressHUDRoundedButton
